function dKudC = op_D_DC_gradu_u_nu_gradv_mp(spu, spv, spg, msh, u, mat, patch_list)

    indices = cell(msh.npatch, 1);
    values = cell(msh.npatch, 1);
    
    for iPatch = patch_list
        [ind, vals] = op_D_DC_gradu_u_nu_gradv_tp(spu.sp_patch{iPatch}, spv.sp_patch{iPatch}, spg.sp_patch{iPatch}, msh.msh_patch{iPatch}, u(spu.gnum{iPatch}), mat);
