function varargout = op_D_DC_DC_gradv_Br_bot(spv, spg, msh, material)
    ...
    b11 = sum(weights.*sum(DleDkdgradNi.*Br_bot, 1).*jacdet, 4);
    b12 = sum(weights.*sum(DkdDlegradNi.*Br_bot, 1).*jacdet, 4);
    b13 = sum(-weights.*sum(DkdgradNi.*Br_bot, 1).*jacdet.*TrDle, 4);

    dbdCdC1 = b11 + b12 + b13;

    b21 = sum(-weights.*sum(DlegradNi.*Br_bot, 1).*jacdet.*TrDkd, 4);
    b22 = sum(weights.*sum(gradNi.*Br_bot, 1).*jacdet.*TrDle.*TrDkd, 4);
    b23 = sum(-weights.*sum(gradNi.*Br_bot, 1).*jacdet.*TrDkdDle, 4);
    ...
    elseif (nargout == 2)
        ...
        error ('op_D_DC_DC_gradv_Br_bot: wrong number of output arguments')
    end
end
