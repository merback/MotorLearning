...
function dKdCdC = op_D_DC_DC_gradu_u_nu_gradv_mp_eval(spu, spuEval, spv, spvEval, spg, spgEval, msh, mshEval, u, material, patch_list)
    ...
    for iPatch = patch_list
         ...
         [ind, vals] = op_D_DC_DC_gradu_u_nu_gradv(spuEval{iPatch}, spvEval{iPatch}, spgEval{iPatch}, mshEval{iPatch}, u(spu.gnum{iPatch}), material);
        ...
        if (~isempty (spg.dofs_ornt))
            warning("op_D_DC_DC_gradu_u_nu_gradv_mp_eval: dofs_ornt not tested yet")
