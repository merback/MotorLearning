...
function dKdCdC = op_D_DC_DC_gradu_u_nu_gradv_mp(spu, spv, spg, msh, u, material, patch_list)
    ...
    for iPatch = patch_list
         ...
         [ind, vals] = op_D_DC_DC_gradu_u_nu_gradv_tp (spu.sp_patch{iPatch}, spv.sp_patch{iPatch}, spg.sp_patch{iPatch}, msh.msh_patch{iPatch}, u(spu.gnum{iPatch}), material);
        ...
        if (~isempty (spg.dofs_ornt))
            warning("op_D_DC_DC_gradu_u_nu_gradv_mp: dofs_ornt not tested yet")
